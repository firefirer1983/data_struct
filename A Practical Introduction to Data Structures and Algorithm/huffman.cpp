...
void read_freqs(heap<HuffTree<char>*, minTreeComp>* forest,
                CodeTable<char>* ct, FILE* fp)
{ // Read a list of strings and frequencies from standard input,
  ...
  while (true) {
    ...
    *ptr2 = '\0'; // End of string
    ...
    // WARNING: This might be considered buggy. Nodes with equal weight will
    // be put in reverse order of appearance, not in alphabetical order.
    forest->insert(temptree);
  }
}

// Build a Huffman tree from sorted linked list "fl"
template <typename E> HuffTree<E>*
buildHuff(heap<HuffTree<E>*,minTreeComp>* fl) {
  HuffTree<E> *temp1, *temp2, *temp3 = NULL;
  while (fl->size() > 1) {
    temp1 = fl->removefirst();   // Pull first two trees  
    temp2 = fl->removefirst();   //   off the list
...
void decode(HuffTree<char>* theTree, char* code, char& msg, int& cnt) {
  ...
  msg = (((LeafNode<char>*)currnode)->val())->value();
}

void buildcode(HuffNode<char>* root, CodeTable<char>* ct,
	       char* prefix, int level, double& total) {
  if (root->isLeaf()) {
    cout << (((LeafNode<char>*)root)->val())->value()
         << "\t" << prefix << "\n";
    strcpy(ct->getcode((((LeafNode<char>*)root)->val())->value()), prefix);
    total += level * (((LeafNode<char>*)root)->val())->key();
  }
  else {
    prefix[level] = '0';
    prefix[level+1] = '\0';
...
int main(int argc, char** argv) {
  ...
  HuffTree<char>* heapspace[2*CODETABLELEN];
  // This heap holds the Huffman forest during construction
  heap<HuffTree<char>*,minTreeComp>* forest =
    new heap<HuffTree<char>*,minTreeComp>(heapspace,0,2*CODETABLELEN);
