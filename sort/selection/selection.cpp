...
public:
  ...
  void SelectionMaxSort(){
		for(int sorted_num=0; sorted_num<size_-1; sorted_num++) {
			int max_idx = sorted_num+1;
  		for(int pos=sorted_num; pos<size_; pos++) {
				if(array_[pos] > array_[max_idx]) {
					max_idx = pos;
				}
  		}
			SWAP(array_[sorted_num], array_[max_idx]);
		}
  };
