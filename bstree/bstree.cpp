  ...
  Node *_Delete(int key, Node *parent) {
    Node *del_node = nullptr;
  	if(key == parent->key_) {
			...
			if(swap_node->left_) {
				end_node = swap_node->left_;
				swap_node->left_ = nullptr;
			} else if(swap_node->right_) {
				end_node = swap_node->right_;
				swap_node->right_ = nullptr;
			}
			if(end_node) {
				cout << "end wrap " << swap_node->key_ << " with " << end_node->key_ << endl;
				swap_node->key_ = end_node->key_;
				delete end_node;
			} else {
				delete swap_node;
			}
			...
			if(parent->left_) {
				del_node = _Delete(key, parent->left_);
			}
  	} else {
			if(parent->right_) {
				del_node = _Delete(key, parent->right_);
			}
  	}
    return del_node;
  }
